...
class VideoConferencingWithBBB {
	 ...
	 * @since    3.0.0
	 * @access   protected
	 * @var      VCBBB_Loader    $loader    Maintains and registers all hooks for the plugin.
	 */
	protected $loader;
	 ...
	 *
	 * - VCBBB_Loader. Orchestrates the hooks of the plugin.
	 * - VCBBB_I18n. Defines internationalization functionality.
	 * - VCBBB_Admin. Defines all hooks for the admin area.
	 * - VCBBB_Public. Defines all hooks for the public side of the site.
	 *
	 ...
	 *
	 * Uses the VCBBB_I18n class in order to set the domain and to register the hook
	 * with WordPress.
	 *
	 * @since    3.0.0
	 * @access   private
	...
	private function define_admin_hooks() {
		...
		$plugin_admin                       = new VCBBB_Admin( $this->get_plugin_name(), $this->get_version() );
		$plugin_admin_api                   = new VCBBB_Admin_Api();
		$plugin_admin_register_custom_types = new VCBBB_Register_Custom_Types();
		$activator                          = new VCBBB_Activator();
		...
		$this->loader->add_filter( 'parse_query', $plugin_admin, 'vcbbb_convert_id_to_term_in_query' );
	...
	/**
	 * Register all of the hooks related to VCBBB_Public the public-facing functionality
	 * of the plugin.
	 *
	 * @since    3.0.0
	 * @access   private
	...
	private function define_public_hooks() {
		...
		$plugin_public               = new VCBBB_Public( $this->get_plugin_name(), $this->get_version() );
		$plugin_public_shortcode     = new VCBBB_Public_Shortcode();
		$plugin_public_room_api      = new VCBBB_Public_Room_Api( $this->get_plugin_name(), $this->get_version() );
		$plugin_public_recording_api = new VCBBB_Public_Recording_Api();
		...
		// Join room API.
		$this->loader->add_action( 'init', $plugin_public_room_api, 'vcbbb_user_join_room' );
		$this->loader->add_filter( 'heartbeat_received', $plugin_public_room_api, 'vcbbb_check_meeting_state', 10, 2 );
		$this->loader->add_filter( 'heartbeat_nopriv_received', $plugin_public_room_api, 'vcbbb_check_meeting_state', 10, 2 );

		// Manage recording state API.
		$this->loader->add_action( 'wp_ajax_set_vcbbb_recording_publish_state', $plugin_public_recording_api, 'set_vcbbb_recording_publish_state' );
		$this->loader->add_action( 'wp_ajax_nopriv_set_vcbbb_recording_publish_state', $plugin_public_recording_api, 'set_vcbbb_recording_publish_state' );
		$this->loader->add_action( 'wp_ajax_set_vcbbb_recording_protect_state', $plugin_public_recording_api, 'set_vcbbb_recording_protect_state' );
		$this->loader->add_action( 'wp_ajax_nopriv_set_vcbbb_recording_protect_state', $plugin_public_recording_api, 'set_vcbbb_recording_protect_state' );
		$this->loader->add_action( 'wp_ajax_trash_vcbbb_recording', $plugin_public_recording_api, 'trash_vcbbb_recording' );
		$this->loader->add_action( 'wp_ajax_nopriv_trash_vcbbb_recording', $plugin_public_recording_api, 'trash_vcbbb_recording' );

		// Edit recording actions.
		$this->loader->add_action( 'wp_ajax_set_vcbbb_recording_edits', $plugin_public_recording_api, 'set_vcbbb_recording_edits' );
		$this->loader->add_action( 'wp_ajax_nopriv_set_vcbbb_recording_edits', $plugin_public_recording_api, 'set_vcbbb_recording_edits' );
	 ...
	 *
	 * @since     3.0.0
	 * @return    VCBBB_Loader    Orchestrates the hooks of the plugin.
	 */
	...
	public function fw_ext_shortcodes_disable_shortcodes( $to_disable ) {
		$to_disable[] = 'video-conferencing-with-bbb';
