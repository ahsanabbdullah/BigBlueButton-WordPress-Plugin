...
class VCBBB_Recording_Helper {
	...
	public function get_recordings_based_on_capability( $room_ids ) {
		...
		if ( $manage_recordings ) {
			$this->recordings = VCBBB_Api::get_recordings( $room_ids, 'published,unpublished,processed,processing' );
	...
	private function filter_managed_recording( $recording ) {
		if ( 'true' == $recording->protected ) {
			...
			$recording->protected_icon_title   = __( 'Protected', 'video-conferencing-with-bbb' );
			...
			$recording->protected_icon_title   = __( 'Unprotected', 'video-conferencing-with-bbb' );
		}
		...
		if ( 'true' == $recording->published ) {
			...
			$recording->published_icon_title   = __( 'Published', 'video-conferencing-with-bbb' );
		} else {
			$recording->published_icon_classes = 'bbb-icon bbb_published_recording not_published dashicons dashicons-hidden';
			$recording->published_icon_title   = __( 'Unpublished', 'video-conferencing-with-bbb' );
		}
	...
	public function order_recordings( $order = '', $order_by = '' ) {
		if ( '' !== $order && '' !== $order_by ) {
			...
			$direction = sanitize_text_field( $order );
			$field     = sanitize_text_field( $order_by );
			$self      = $this;
