<div id="vcbbb-recordings-list-<?php echo esc_attr( $room_id ); ?>">
	...
	<?php } else { ?>
		...
		<div id="vcbbb-recordings-table" class="vcbbb-table-container" role="table">
			<div class="vcbbb-flex-table vcbbb-flex-table-<?php echo $columns; ?> vcbbb-header" role="rowgroup">
				...
				<?php if ( $manage_vcbbb_recordings ) { ?>
			...
			<?php foreach ( $recordings as $recording ) { ?>
				<div id="vcbbb-recording-<?php echo esc_attr( $recording->recordID ); ?>" class="vcbbb-flex-table vcbbb-flex-table-<?php echo esc_attr( $columns ); ?> vcbbb-recording-row" role="rowgroup">
						...
						<div id="vcbbb-recording-links-block-<?php echo esc_attr( $recording->recordID ); ?>" class="vcbbb-recording-link-block" style="<?php echo ( $recording->published == 'false' ? 'display:none;' : '' ); ?>">
							<?php foreach ( $recording->playback->format as $format ) { ?>
								...
								if ( $format->type == $default_bbb_recording_format || $view_extended_recording_formats ) {
									$recording_url = trim( apply_filters( 'vcbbb_recording_url_display', $format->url, $format->type ) );
									?>
									<button class="vcbbb-button vcbbb-btn-join button button-primary" onclick="window.open('<?php echo esc_url( $recording_url ); ?>', '_blank')"><?php esc_html_e( 'View Recording', 'video-conferencing-with-bbb' ); ?></button>								<?php break; // Show only the first format ?>								<?php } ?>
							<?php } ?>
						</div>
					</div>
					<?php if ( $manage_vcbbb_recordings ) { ?>
						<div class="flex-row flex-row-<?php echo $columns; ?>" role="cell">
							...
							<i data-record-id="<?php echo esc_attr( $recording->recordID ); ?>"
                                ...
                                aria-label="<?php echo esc_attr_x( 'Trash', 'post status', 'video-conferencing-with-vcbbb' ); ?>">
							&nbsp;
